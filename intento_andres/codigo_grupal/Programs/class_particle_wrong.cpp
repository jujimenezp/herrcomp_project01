...
void Particle::Move_hole(const int &step, const int &direction, const int &particle_id, const CONFIG &config, std::vector<int> &Cells, std::vector<Particle> &Particles)
{
  ...
  else if (direction == 0 && config.holeposition == (step + 1)-1){ //the particle is on the limit after moving and moved towards the hole
  ...
  else if (direction == 1 && config.holeposition == -(-step + 2)+3){ //the particle is on the limit after moving and moved towards the hole
